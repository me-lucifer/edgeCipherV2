...
const postureSuggestions: Record<VixZone, Record<PersonaType, VixAdvice>> = {
    "Extremely Calm": {
        "Impulsive Sprinter": { interpretation: "Calm markets can feel slow, triggering your impulse to force trades. Patience is your primary edge today. Avoid chasing small moves and wait for A+ setups to preserve mental capital.", rule: "Do not invent setups; wait for the market to present a clear opportunity.", actions: [{label: "Stick to high-probability setups"}, {label: "Respect range boundaries"}, {label: "Use this quiet time for journal review", module: "tradeJournal"}] },
        "Fearful Analyst": { interpretation: "Low-stress conditions are perfect for building confidence. Observe and plan without pressure. Build conviction in your analysis before risking capital by identifying key levels and ideas.", rule: "Focus on planning and analysis, not forced execution.", actions: [{label: "Build confidence by paper trading setups"}, {label: "Identify clear range levels", module: "chart"}, {label: "Plan entries/exits without emotional pressure", module: 'tradePlanning'}] },
        "Disciplined Scalper": { interpretation: "Range-bound strategies can work well now. Your discipline is key to avoid over-leveraging on small moves. Execute your plan, but cut trades that don't show immediate follow-through.", rule: "Execute your plan, but protect your capital from choppy price action.", actions: [{label: "Focus on clear S/R flips"}, {label: "Take profits at defined levels"}, {label: "Monitor for signs of increasing volatility"}] },
        "Beginner": { interpretation: "Ideal environment to study market structure without high risk. Watch price at key levels and practice planning without executing. Your main goal is to observe and learn, not to trade.", rule: "Today is for learning and observation, not for aggressive trading.", actions: [{label: "Practice identifying support and resistance", module: "chart"}, {label: "Observe how price reacts at key levels"}, {label: "Formulate a simple trade plan without executing", module: "tradePlanning"}] },
    },
    "Normal": {
        "Impulsive Sprinter": { interpretation: "Normal conditions can still trigger impatience. Your biggest risk is deviating from your plan. The goal is consistent execution. Stick to your rules, respect stops, and journal every trade.", rule: "Follow your plan without exception; no adding to losers or revenge trading.", actions: [{label: "One trade at a time"}, {label: "Respect your stop-loss, no exceptions"}, {label: "Complete journal before your next trade", module: "tradeJournal"}] },
        "Fearful Analyst": { interpretation: "Good conditions for well-planned trades. Your analysis is sound; trust it. Hesitation is costly. Execute your plan with confidence. Start with a smaller size if needed.", rule: "Trust your analysis and execute your A+ setups with confidence.", actions: [{label: "Trust the analysis you did in calmer times"}, {label: "Use a pre-flight checklist before entering"}, {label: "Start with a smaller-than-normal position size"}] },
        "Disciplined Scalper": { interpretation: "Prime conditions. The market has direction but isn't overly chaotic. Your edge is sharpest now. Focus on disciplined execution. Don't get complacent; stick to profit-taking rules and watch for volatility shifts.", rule: "Execute your A+ setups without hesitation and adhere to your rules.", actions: [{label: "Execute A+ setups cleanly"}, {label: "Adhere strictly to your profit-taking rules"}, {label: "Monitor for signs of increasing volatility"}] },
        "Beginner": { interpretation: "Good environment to practice. Focus on executing one or two simple setups with very small, controlled size. The goal is to learn the process of planning, executing, and journaling a trade.", rule: "Practice disciplined execution with a minimal position size.", actions: [{label: "Focus on one or two simple setups"}, {label: "Practice setting entry, stop, and profit orders", module: "tradePlanning"}, {label: "Journal every action and emotion immediately", module: "tradeJournal"}] },
    },
    "Volatile": {
        "Impulsive Sprinter": { interpretation: "Danger zone for you. Volatility can feel like endless opportunities, but your impulses are costly here. Your job is defense. Reduce size significantly and be selective. If no A+ setup, don't trade.", rule: "Your first priority is capital protection, not profit.", actions: [{label: "Cut position size by 50%", module: 'tradePlanning', context: { safeMode: true } }, {label: "Wait for crystal-clear A+ setups"}, {label: "Set a hard stop on number of trades", module: 'riskCenter'}] },
        "Fearful Analyst": { interpretation: "Analysis is difficult in choppy markets. It is acceptable to sit out. Protecting mental capital and avoiding bad experiences is key. A flat day is a winning day in these conditions.", rule: "If you are not 100% confident in a setup, the best trade is no trade.", actions: [{label: "If unsure, stay flat"}, {label: "Drastically reduce size if you see a perfect setup"}, {label: "Watch price action without pressure", module: 'chart'}] },
        "Disciplined Scalper": { interpretation: "Your strategy is at high risk. Wider wicks and slippage can easily stop you out. Adapt by widening stops, reducing size, or waiting. Show discipline by not forcing a strategy that doesn't fit the environment.", rule: "Adapt your parameters for volatility or wait for calmer conditions.", actions: [{label: "Widen stops slightly to avoid getting wicked out"}, {label: "Reduce position size significantly", module: 'tradePlanning', context: { safeMode: true } }, {label: "Focus on setups with very clear invalidation"}] },
        "Beginner": { interpretation: "Sit on your hands today. Watching the chaos from the sidelines is the most valuable lesson. You cannot predict these moves. This is a live demo of why risk management matters. Observe, learn, and preserve capital.", rule: "Observe the market, do not participate.", actions: [{label: "Watch how price interacts with key levels", module: 'chart'}, {label: "Notice how quickly moves can reverse"}, {label: "See this as a live lesson in risk management"}] },
    },
    "High Volatility": {
        "Impulsive Sprinter": { interpretation: "'Red alert' for your persona. The market is erratic, and your impulse to trade is dangerous. Risk of significant loss is extremely high. Trading now is gambling. Protect your capital by not participating.", rule: "Your only job today is to protect your capital by not trading.", actions: [{label: "Close the charts for a few hours"}, {label: "Read your trading plan instead", module: 'strategyManagement'}, {label: "A flat day is a winning day in these conditions"}] },
        "Fearful Analyst": { interpretation: "Analysis is unreliable now. The market is driven by liquidations and fear. Protect your capital and mindset by staying flat. The pros are waiting, and so should you.", rule: "Stay flat. Pros are waiting, and so should you.", actions: [{label: "Do not feel pressure to trade"}, {label: "This is a good time for high-level review", module: 'analytics'}, {label: "Study how 'black swan' events unfold"}] },
        "Disciplined Scalper": { interpretation: "Your edge does not exist here. The noise is too high and risk is undefinable. Your discipline is shown by not participating until the market normalizes. Any trade now is a gamble. Preserve your capital.", rule: "Cash is the strongest position right now. Preserve your capital.", actions: [{label: "Preserve capital; opportunity will come later"}, {label: "Avoid the temptation to catch falling knives"}, {label: "Review your rules for high-volatility", module: 'strategyManagement'}] },
        "Beginner": { interpretation: "This is where new traders lose their accounts. Do not trade. Your job is to watch from a distance and understand that no action is the best action. Survival is the only goal. This experience from the sidelines is invaluable.", rule: "Do not trade. Your goal is to survive to trade another day.", actions: [{label: "Observe the chaos from the sidelines"}, {label: "Understand that you cannot predict these moves"}, {label: "Learn that sometimes the best action is no action"}] },
    },
    "Extreme": {
        ...
        "Fearful Analyst": { interpretation: "The market is irrational. Your analysis does not apply. Do not try to find a bottom or top. Confirm positions are closed or protected and wait for sanity to return. Your fear is justified; trust it and stay flat.", rule: "Stay flat. This is a spectator sport right now.", actions: [{label: "Confirm all open positions are closed or protected"}, {label: "Read a book on trading psychology"}, {label: "Wait for volatility to return to normal levels"}] },
        "Disciplined Scalper": { interpretation: "No edge here. The market is in a liquidation cascade. Any position is a gamble. Your discipline requires you to stay flat and protect your capital. This is not the environment for your strategy.", rule: "Stay flat. There is no trading edge in a liquidation cascade.", actions: [{label: "Stay flat and protect your capital"}, {label: "Wait for volatility to return to normal levels"}, {label: "This is a day for risk managers, not traders"}] },
...
function VolatilityBriefCard({ vixState, persona }: { vixState: VixState, persona: PersonaType | null }) {
    ...
    const generateReportText = (short = false): string => {
        ...
        if (short) {
            const shortRecommendation = posture?.rule || doDontSuggestions[currentZone].dos[0];
